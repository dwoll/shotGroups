...
\textbf{Maßnahme}: Die Pakete \lstinline!renv!\index[pack]{renv@\lstinline{renv}} \cite{Ushey2019} und \lstinline!checkpoint!\index[pack]{checkpoint@\lstinline{checkpoint}} \cite{Microsoft2016b} liefern unterschiedliche Ansätze, um die verwendeten Pakete für ein Auswertungsprojekt festzuhalten und auf einen definierten projektspezifischen Versionsstand fixieren zu können.
...
\textbf{Maßnahme}: Auswertungsskripte und R-Dokumente (Abschn.\ \ref{sec:rmd}) können mit aus der Softwareentwicklung stammenden \emph{Versionskontrollsystemen} versioniert werden. Besonders gut ist die Software git in RStudio integriert.\footnote{\url{https://git-scm.com/} -- Tutorials finden sich unter \url{https://happygitwithr.com/} und bei \citeA[Kap.~13]{Wickham2014e}: \url{http://r-pkgs.had.co.nz/git.html}} Mit git ist es möglich, den Zustand von Auswertungsskripten oder R-Markdown Dokumenten zu bestimmten Zeiten {\quotedblbase}einzufrieren{\textquotedblleft} und auch nach späteren Änderungen wieder auf diesen Zustand zurückzusetzen.
...
\item Originaldaten sind von abgeleiteten Daten zu trennen, die durch Datenaufbereitung zustande kommen. Insbesondere sollten manuelle Korrekturen durch Auswertungsskripte erfolgen und nicht in Datensätzen vorgenommen werden.
